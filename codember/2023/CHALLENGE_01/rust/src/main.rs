...
fn count_word_repetitions(message: &str) -> String {
    ...
    for word in message.split_whitespace() {
        if words_seen.insert(word, ()).is_none() {
...
fn main() {
    let repetitions = count_word_repetitions(MESSAGE);

    println!("Word repetitions: {}", repetitions);
}
